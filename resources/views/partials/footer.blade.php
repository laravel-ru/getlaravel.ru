            ...
            <div class="relative max-w-screen-xl mx-auto px-8">
                <section class="relative z-10 p-6 bg-white shadow-lg md:flex md:items-center md:p-12 lg:p-16 {{ $is_dark ? 'dark:bg-dark-600' : '' }}">
                    <div class="content md:pr-12">
                        <h2 class="text-3xl tracking-tight sm:text-4xl md:mt-4 xl:text-5xl {{ $is_dark ? 'dark:text-gray-200' : '' }}">Станьте партнером Laravel</h2>
                        <p class="mt-3 max-w-xl text-gray-600 sm:mt-5 md:max-w-2xl md:mt-8 md:text-lg {{ $is_dark ? 'dark:text-gray-400' : '' }}">Партнеры Laravel - это элитные магазины, предлагающие первоклассную разработку и консультирование на Laravel. Каждый из наших партнеров может помочь Вам создать красивый, хорошо продуманный проект.</p>
                    </div>
            ...
            <div class="px-8 pb-12 bg-gray-100 xl:px-20 {{ $is_dark ? 'dark:bg-dark-900' : '' }}">
                ...
                <div class="mt-6 sm:mt-12 md:flex">
                    ...
                    <div class="mt-10 max-w-md md:mt-0 md:w-1/2 lg:w-1/4">
                        <p class="text-xs text-gray-600 sm:text-sm {{ $is_dark ? 'dark:text-gray-500' : '' }}">Laravel - это фреймворк для веб-приложений с выразительным элегантным синтаксисом. Мы считаем, что развитие должно быть приятным и творческим, чтобы приносить истинное удовлетворение. Laravel пытается облегчить разработку, упростив общие задачи, используемые в большинстве веб-проектов.</p>
                        <p class="mt-6 text-xs text-gray-600 text-opacity-75 sm:text-sm {{ $is_dark ? 'dark:text-gray-500' : '' }}">
                            Laravel является торговой маркой Тейлора Отвелла.<br>Авторское право &copy; 2011-{{now()->format('Y')}} Laravel LLC.
                        </p>
